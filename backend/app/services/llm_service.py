...
MODEL = "meta/llama-3.1-405b-instruct"  # Since deepseek-v4-flash is not available on nvidia nim yet, fallback to llama or let's just stick to the prompt structure  # noqa
...
async def generate_dpdp_narrative(clauses: list[dict]) -> str:
    ...
    response = await client.chat.completions.create(
        ...
        messages=[
            {
                "role": "system",
                "content": "Always include a disclaimer that this is a readiness indicator, not legal certification.",  # noqa
            },
            {
                "role": "user",
                "content": f"""
