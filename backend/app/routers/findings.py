...
async def list_findings(
    ...
    if severity: count_stmt = count_stmt.where(Finding.severity == severity)
    if status: count_stmt = count_stmt.where(Finding.status == status)
    if finding_type: count_stmt = count_stmt.where(Finding.finding_type == finding_type)
    if asset_id: count_stmt = count_stmt.where(Finding.asset_id == asset_id)
...
@router.put("/{finding_id}/status", response_model=FindingResponse, dependencies=[Depends(require_role("owner", "admin", "member"))])
async def update_finding_status(
    ...
    if payload.status == "false_positive" and not payload.false_positive_reason:
        raise HTTPException(status_code=422, detail="false_positive requires a false_positive_reason")
