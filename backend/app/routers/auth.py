...
async def register(
    ...
    async with httpx.AsyncClient() as client:
        ...
        if not user_id:
             raise HTTPException(status_code=500, detail="Failed to retrieve user ID from Auth provider")
        ...
        update_resp = await client.put(
            ...
            headers=headers,
            json={
                "app_metadata": {"org_id": str(org.id)}
            }
        )
        ...
        # Optionally login to return JWTs immediately if Supabase didn't return session data in signup
