...
@router.put("", response_model=OrgProfileResponse, dependencies=[Depends(require_role("owner", "admin"))])
...
@router.delete("", status_code=status.HTTP_202_ACCEPTED, dependencies=[Depends(require_role("owner"))])
...
@router.post("/assets", response_model=AssetResponse, dependencies=[Depends(require_role("owner", "admin"))])
async def add_asset(
    payload: AssetCreate,
    current_org: Annotated[CurrentOrg, Depends(get_current_org)],
    db: AsyncSession = Depends(get_db_session),
...
@router.post("/domain/verify-token", response_model=VerifyTokenResponse, dependencies=[Depends(require_role("owner", "admin"))])
...
@router.post("/domain/verify-check", response_model=MessageResponse, dependencies=[Depends(require_role("owner", "admin"))])
