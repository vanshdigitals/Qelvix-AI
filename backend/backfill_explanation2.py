...
async def backfill():
    ...
    stmt = select(Finding).where(Finding.finding_type == 'email_breached').order_by(Finding.discovered_at.desc()).limit(1)
    result = await db.execute(stmt)
    finding = result.scalars().first()
    
    if finding:
        print(f"Found finding: {finding.id} ({finding.title})")
