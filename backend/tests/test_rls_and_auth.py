...
def create_mock_jwt(user_id: str, org_id: str) -> str:
    payload = {"sub": user_id, "org_id": org_id, "aud": "authenticated"}
...
async def test_auth_rejection(async_client: AsyncClient):
    ...
    resp_invalid = await async_client.get(
        "/org/me", headers={"Authorization": "Bearer invalid.token.here"}
    )
...
async def test_rls_cross_tenant():
    ...
    async with engine.connect() as conn:
        ...
        async with conn.begin():
            ...
            await conn.execute(
                sa.text(f"SET LOCAL request.jwt.claim.org_id = '{jwt_claim_org_id}'")
            )
            ...
            logger.info("\n--- RLS TEST OUTPUT ---")
            logger.info(f"Finding inserted for Org A: {finding_id}")
            logger.info(f"Querying as Org B ({org_b_id}) returned {len(findings_for_b)} findings.")
            ...
            logger.info(f"Querying Org A directly as Org B returned: {org_a_visible}")
    ...
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as ac:
        ...
        logger.info(f"Auth Test (Valid Token): GET /org/me -> Status {resp_valid.status_code}")
